...
function makeBall(el, x, y, type='object', number=null){
  ...
  const b={el,x,y,vx:0,vy:0,type,number:(color==='black'?8:number),color,pocketed:false,sleeping:false,shine,streak,ring,rollAngle:0,lastRollPx:null,lastRollPy:null};
  balls.push(b);
  return b;
}
...
function setBallPos(b){
  ...
  // RODADURA REAL VISTA DESDE ARRIBA:
  // La textura/numero de la bola gira según la distancia recorrida.
  // Una vuelta completa corresponde aproximadamente a la circunferencia
  // de la esfera (angulo = distancia / radio). Esto evita una rotación
  // artificial ligada al tiempo: si la bola se mueve más, gira más.
  const cx=b.x*w, cy=b.y*h;
  if(b.lastRollPx===null || b.lastRollPy===null){
    b.lastRollPx=cx; b.lastRollPy=cy;
  }else{
    const dx=cx-b.lastRollPx, dy=cy-b.lastRollPy;
    const distance=Math.hypot(dx,dy);
    const r=Math.max(1,ballRadius());
    if(distance>0.001){
      // El signo se determina por la dirección de desplazamiento.
      // Para una esfera que avanza, la textura rota sobre el eje perpendicular.
      const direction = Math.abs(dx) >= Math.abs(dy)
        ? (dx >= 0 ? -1 : 1)
        : (dy >= 0 ? 1 : -1);
      b.rollAngle += direction*(distance/r);
      b.lastRollPx=cx; b.lastRollPy=cy;
    }
  }
  ...
  b.el.style.top=top;
  b.el.style.transform=`translate(-50%,-50%) rotate(${b.rollAngle}rad)`;
  ...
  if(b.shine && b.streak && b.ring){
    ...
    if(q>0.035 && !b.pocketed){
      ...
      const offset=2+q*7;
      const sx=50+Math.cos(b.rollAngle)*14+backX*offset, sy=50+Math.sin(b.rollAngle)*14+backY*offset;
...
function updateAim(forcedAngle=null){
  ...
  // LÍMITES DE LA GUÍA = LÍMITES AMARILLOS DE "BOLA EN MANO".
  // IMPORTANTE: NO usar editableLimits ni customLimitSegments aquí.
  // Esos son los límites rojos/físicos de las bolas y no deben afectar
  // la guía de apuntado. La guía debe obedecer exclusivamente al marco
  // amarillo transparente de Limites_Bola_En_Mano.png.
  //
  // El PNG mide 1997x1047 y el trazo amarillo ocupa x=98..1903,
  // y=98..951. Como el overlay se estira exactamente al tamaño de la mesa,
  // estas proporciones son los mismos límites visuales en cualquier tamaño.
  const YELLOW_HAND_LIMITS={
    // Borde INTERIOR de la línea amarilla. La línea mide ~13 px;
    // al usar su borde interior, el círculo puede llegar hasta ella
    // sin meterse dentro ni dejar una separación artificial.
    left:110/1997,
    right:1890/1997,
    top:110/1047,
    bottom:938/1047
  ...
  {
    const left=YELLOW_HAND_LIMITS.left*gw;
    const right=YELLOW_HAND_LIMITS.right*gw;
    const top=YELLOW_HAND_LIMITS.top*gh;
    const bottom=YELLOW_HAND_LIMITS.bottom*gh;

    // Igual que al colocar la blanca con bola en mano: el límite corresponde
    // al BORDE de la blanca, por lo que el centro de la blanca/guía se detiene
    // a un radio de distancia de la línea amarilla.
    const yellowEdges=[
      {axis:'x',value:left,a:top,b:bottom},
      {axis:'x',value:right,a:top,b:bottom},
      {axis:'y',value:top,a:left,b:right},
      {axis:'y',value:bottom,a:left,b:right}
    ];

    for(const edge of yellowEdges){
      if(edge.axis==='x'){
        if(Math.abs(ux)<1e-8) continue;
        // Probamos el lado que realmente encuentra el rayo. El centro debe
        // llegar hasta la línea amarilla menos/plus el radio, no hasta el
        // límite rojo transparente.
        const targetX=ux>0 ? edge.value-r : edge.value+r;
        const t=(targetX-c.x)/ux;
        if(t<=0 || !Number.isFinite(t)) continue;
        const yy=c.y+uy*t;
        if(yy>=edge.a-r && yy<=edge.b+r) wallCandidates.push(t);
      }else{
        if(Math.abs(uy)<1e-8) continue;
        const targetY=uy>0 ? edge.value-r : edge.value+r;
        const t=(targetY-c.y)/uy;
        if(t<=0 || !Number.isFinite(t)) continue;
        const xx=c.x+ux*t;
        if(xx>=edge.a-r && xx<=edge.b+r) wallCandidates.push(t);
      }
    }
  }
...
function physics(dt){
  ...
  for(const seg of customLimitSegments){
    ...
    // Las puntas de los segmentos que llegan a una tronera NO son esquinas
    // físicas. Antes el cálculo con u clamped a [0,len] convertía cada extremo
    // en una pequeña pared circular y esas "esquinitas" bloqueaban el paso de
    // la bola aunque la abertura de la tronera estuviera libre.
    // Si el punto de contacto está dentro de la zona de entrada de una tronera,
    // dejamos que pocketCheck() sea quien decida si la bola entra o continúa.
    const pocketCornerGap=Math.max(pRForPhysics(w)+R*1.20, R*2.10);
    let nearPocketOpening=false;
    for(const [pcx0,pcy0] of pocketPoints(w,h)){
      if(Math.hypot(cx-pcx0,cy-pcy0) < pocketCornerGap ||
         Math.hypot(pcx-pcx0,pcy-pcy0) < pocketCornerGap){
        nearPocketOpening=true;
        break;
      }
    }
    if(nearPocketOpening) continue;
  ...
  if(vn<0){
    ...
    let rvx=(vx-2*vn*nx)*wb;
    let rvy=(vy-2*vn*ny)*wb;
      ...
      if(a===balls[0] || b===balls[0]){
        ...
        if(wasTargetStill){
          ...
          if(target===firstShotTarget){
            // IMPORTANTE: el efecto de la blanca NO puede modificar la
            // dirección de la bola objetivo. Cuando hay efecto, dejamos que
            // el choque real (la normal entre los centros en el instante del
            // contacto) determine por completo la dirección de salida de la
            // bola objetivo. La guía inicial solo se usa como ayuda visual,
            // nunca como una orden de dirección para la bola golpeada.
  ...
  if(moving){
    ...
    if(!movingBalls){
      ...
      if(pendingWhiteRespawn){
        ...
        placementHeld=false; placementPointerId=null; placementSideSigns=null;
        placeCueBtn.classList.add('active');
        aimLine.style.display='none'; aimDot.style.display='none'; impactLine.style.display='none'; if(cueExitLine) cueExitLine.style.display='none';
        const ac=document.querySelector('#aimCue');
        if(ac) ac.style.visibility='hidden';
